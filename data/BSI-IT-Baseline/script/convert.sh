...
jq '."ns3:syncRequest".syncData.syncObject | .extId? as $extId | .extObjectType as $parentType | .children[] | .extId as $childId | .extObjectType as $childType | {parentId: $extId, parentType: $parentType, childId: $childId, childType: $childType}' build/verinice.json > build/child_links-l1.json

# level 2:
jq '."ns3:syncRequest".syncData.syncObject.children[] | .extId as $parentId | .extObjectType as $parentType | .children[] |  .extId? as $childId | .extObjectType as $childType | {parentId: $parentId, parentType: $parentType, childId: $childId, childType: $childType}' build/verinice.json > build/child_links-l2.json

# level 3:
jq '."ns3:syncRequest".syncData.syncObject.children[].children[] |  .extId? as $parentId | .extObjectType as $parentType | .children[] | .extId as $childId | .extObjectType as $childType | {parentId: $parentId, parentType: $parentType, childId: $childId, childType: $childType}' build/verinice.json > build/child-links-l3.json

# level 4:
jq '."ns3:syncRequest".syncData.syncObject.children[].children[].children? | select(.) | .[] | select(.) | .extId as $parentId | .extObjectType as $parentType | .children? | select(.) | .[] | select(.) | .extId? as $childId | .extObjectType? as $childType | {parentId: $parentId, parentType: $parentType, childId: $childId, childType: $childType}' build/verinice.json > build/child_links-l4.json

# level 5:
jq '."ns3:syncRequest".syncData.syncObject.children[].children[].children? | select(.) | .[] | select(.) | .children? | select(.) | .[] | select(.) | .extId? as $parentId | .extObjectType? as $parentType | .children? | select(.) | .[] | select(.) | .extId? as $childId | .extObjectType? as $childType | {parentId: $parentId, parentType: $parentType, childId: $childId, childType: $childType}' build/verinice.json > build/child_links-l5.json
